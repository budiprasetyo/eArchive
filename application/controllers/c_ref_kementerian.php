 ...
 * 
 */
class C_ref_kementerian extends CI_Controller
{
	private $limit = 10;
	/**
	...
	public function __construct()
	{
		...
		// initialize table
		$this->m_base->initialize('r_menteri');
		// log in checking
	...
	public function index($offset = 0)
	{
		...
		// load data
		$kementerians = $this->m_base->get_paged_list('id',$this->limit, $offset)->result();

		// generate pagination
		$this->load->library('pagination');
		$config['base_url'] = site_url('c_ref_kementerian/index/');
		...
		$this->table->set_heading('No.','Kementerian','Eselon 1','Eselon 2','Action');
		$i = 0 + $offset;
		foreach ($kementerians as $kementerian)
		{
			$this->table->add_row(++$i.".", $kementerian->kdmenteri."<br />".$kementerian->nmmenteri, $kementerian->kdes1."<br />".$kementerian->nmes1, $kementerian->kdes2."<br />".$kementerian->nmes2, 
	...
	public function view($id)
	{
		...
		$data['kementerian'] = $this->m_base->get_by_id('id',$id)->row();
	...
	public function update($id)
	{
		...
		// call the query
		$kementerian = $this->m_base->get_by_id('id',$id)->row();
		// prefill form values
		...
		$this->form_data->kdes2 	= substr($kementerian->kdes2,2,2);
	...
	public function updateKementerian()
	{
		...
		else
		{
			...
			$this->m_base->update('id',$id,$kementerian);
	...
	public function _set_rules()
	{
		...
		$this->form_validation->set_rules('kdes2','Kode Eselon 2','trim|required|min_length[2]|callback_field_check[r_menteri.kdmenteri.'.$this->input->post('kdmenteri').'.kdes1.'.$this->input->post('kdes1').'.kdes2]');
	 ...
	 * field check
	 * ---------------------------------------------------------------*/
	public function field_check($value, $field_name)
	{
		$parts = explode('.', $field_name);
		// array of where, carefully when define these arrays
		$where_array = array( 	$parts[1] => $parts[2],
								$parts[3] => $parts[4],
								$parts[5] => $parts[4].$value
		);
		
		if( $query = 	$this->m_base->get_where($where_array,1) )
		{
			if($query->num_rows() > 0)
			{
				$this->form_validation->set_message('field_check','Data dengan kode kementerian: '.$parts[2].' kode eselon I: '.$parts[4].' kode eselon II: '.$parts[4].$value.' sudah ada');
				return FALSE;
			}
			else
			{
		...
		{
			$this->form_validation->set_message('field_check','Query gagal');
			return FALSE;
		}
	}
...
// ------------------------------------------------------------------------
// End of C_ref_kementerian Controller Class.
// ------------------------------------------------------------------------
/* End of file c_ref_kementerian.php */
/* Location: ../application/controllers/c_ref_kementerian.php */
